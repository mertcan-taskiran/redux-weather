    ...
    <Container className='border'>
      { items && <>
        <Row>
            ...
            <Col className='mt-5 '>
               <h4>Bugün</h4>
               <p><small>Nem : </small> {items.current.humidity}%</p>
               <p><small>Rüzgar Hızı : </small>{Math.round(items.current.wind_speed)}mph</p>
            </Col>
          </Row> 
                                  ...
                                  <Col key={item.dt} className='card'>
                                    ...
                                    <h5> {date.toUTCString().slice(0,3)}</h5>
                                      ...
                                      <Card.Body>
                                        ...
                                        <Card.Text className='fs-sm-5'><span >En yüksek {Math.round(item.temp.max)}°C <br/>En düşük {Math.round(item.temp.min)}°C</span></Card.Text>
                                      </Card.Body>
                                    </Card>
                                  </Col>
                                 )
